...
export default function FormEditProducts({
	...
	const form = useForm<z.infer<typeof PostProductFrontSchema>>({
		...
		defaultValues: {
			...
			carBrandId: dataProduct?.carBrand?.id ?? "",
			performance: dataProduct?.performance ?? [],
			safety: dataProduct?.safety ?? [],
			interior: dataProduct?.interiorImage ?? [],
			exterior: dataProduct?.exteriorImage ?? [],
			technology: dataProduct?.technology ?? [],
